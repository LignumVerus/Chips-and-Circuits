...
def astar_algorithm(
    ...
    # (route, distance)
    best_unfinished_child = (
        [start_coordinate],
        manhattan_distance(start_coordinate, end_coordinate),
    )
    ...
    while not q.empty():
        ...
        for i in valid_directions(
            ...
            unfinished_route_costs.append(
                (child, f_value(board, child, i[0], end_coordinate, i[1]))
            )

        # put child with lowest cost
        if len(unfinished_route_costs) > 0:
            best_unfinished_child, best_child_path = find_best_child(
                unfinished_route_costs, end_coordinate, best_unfinished_child
            )
    ...
    for i in range(shuffles):
        ...
        for index in all_indexes_routes:
            for cost in combis:
                ...
                netlist, board = find_new_line(
                    netlist, index, chips_dict, board_size, board, wind, up, down
                )
...
def solve_empty_routes(netlist, chips_dict, board_size, board):
    ...
    for line in netlist:
        if not line.route:
            ...
            line.route = find_route(
                start_coordinate, end_coordinate, chips_dict, board_size, board, overlap
            )[0]
...
def closest_line_index(
    route, netlist, end_coordinate, chips_dict, board_size, board, overlap
):
    """
    ...
    for index, line in enumerate(netlist):
        closest_point = closest_point_per_line(
            line, x_range, y_range, route[-1], chips_dict, board_size, board, overlap
        )
...
def closest_point_per_line(
    ...
    for point in line.route:
        ...
        if (point[0] >= x_range[0] and point[0] <= x_range[1]) and (
            point[1] >= y_range[0] and point[1] <= y_range[0]
        ):
            # make sure the point is reachable from the current point
            found = find_route(
                coordinate, point, chips_dict, board_size, board, overlap
            )
